...
library(tidyr)
library(readxl)
library(gridExtra)  # 用于图形拼接

# 读入Sheet1数据（A图数据）
data_sheet1 <- read_excel("D:/4生成AI/医生-AI-调查问卷/可视化/02_百分比柱状图.xlsx", sheet = 1)

# 将Sheet1数据转换为长格式并计算百分比
...
p_A <- ggplot(long_data_sheet1, aes(x = Name, y = Percentage, fill = Category)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 0.5, size = 13, color = "black"),
        ...
        axis.ticks.y = element_line(color = "black", size = 1), # 添加x轴ticks
        ...
        text = element_text(color = "black")) +  # 设置所有文本为黑色
  geom_text(aes(label = Label), 
            position = position_fill(vjust = 0.5), hjust = 0.5, size = 4.5, color = "black") +  # 设置geom_text文本为黑色
...
# 读入Sheet2数据（B图数据）
data_sheet2 <- read_excel("D:/4生成AI/医生-AI-调查问卷/可视化/02_百分比柱状图.xlsx", sheet = 2)

# 将Sheet2数据转换为长格式并计算百分比
...
# 将A图和B图纵向拼接在一起
grid.arrange(p_A, p_B, ncol = 1, nrow = 2, heights = c(4, 4))  # heights参数可以调整两个图的高度比例

# 保存拼接后的图形为PDF文件
pdf("D:/4生成AI/医生-AI-调查问卷/可视化/02_combined_percentage_bar_charts.pdf", width = 8, height = 12)
