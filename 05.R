...
mydata_clinical <- read_excel("05_concern.xlsx") %>%
...
p_clinical <- ggplot(mydata_clinical, aes(y = class, x = adjusted_value, fill = type)) + 
  ...
  theme(
    ...
    axis.title.y = element_text(vjust = 5, color = "black", size = 20, face = "bold", margin = margin(t = 10)),
    axis.line.x = element_line(color = "black", arrow = arrow(length = unit(0.2, "cm"), type = "closed", ends = "both"), linewidth = 0.8),
    legend.title = element_text(vjust = 1, size = 15, face = "bold", margin = margin(t = 5)),
